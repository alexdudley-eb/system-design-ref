...
export function getFlashcardHistory(): FlashcardSession[] {
  try {
    ...
    const sessions = JSON.parse(stored) as FlashcardSession[]
    return sessions.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())
  } catch (error) {
...
export function getTestHistory(): TestSession[] {
  try {
    ...
    const sessions = JSON.parse(stored) as TestSession[]
    return sessions.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())
  } catch (error) {
...
export function calculateScore(scores: AssessmentScores): {
  ...
  const totalPoints = allScores.reduce((sum, item) => sum + scoreValues[item.score], 0)
